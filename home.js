...
document.addEventListener("DOMContentLoaded", async function () {
    try {
        const url = await fetch("bicicletas.json");
        const data = await url.json();

        for (const bici of data.bicicletas) {
            ...
            card.innerHTML = `
            <img src="${bici.imagen}"  alt="...">
            <div class="card-body" >
                <h5 class="card-title">${bici.modelo}</h5>
                <h6 class="card-title">${bici.marca}</h6>
                <p class="card-text">$ ${bici.precio}</p>
                <button class="comprar" data-id="${bici.id}" id="comprar">Añadir al carro</button>
            </div>
            `;
...
filtro1.addEventListener('click', () => cargarBicicletas("mountain"));
filtro2.addEventListener('click', () => cargarBicicletas("trekk"));
filtro3.addEventListener('click', () => cargarBicicletas("road"));

async function cargarBicicletas(categoria) {
...
const agregarAlCarrito = async (id) => {
    try {
        const url = await fetch("bicicletas.json");
        const data = await url.json();
        let bicis = data.bicicletas;

        const productoEnTienda = bicis.find(producto => producto.id === id);
